...
import { CART_URL } from "../config/globalVariable";    
    ...
    function AuthenCart(){
        let render=useRef(0)
        render.current++
        console.log("renderTimes:"+render.current);
        ...
        console.log("useEffectTimes: "+ RenderCount.current);
    ...
    const getAuthenCart = async () =>{
        try {
            const respone = await axious.get(`${CART_URL}/getCartItem/${id}`,
            {
                headers:{
            ...
            <section className="container stylization maincont">
                ...
                <form >
                    ...
                    <ul className="cart-total">
                        <li className="cart-summ">Tổng : <b id="cost">{total}</b></li>
                    </ul>
                    <div className="cart-submit">
                        
                        <button className="cart-submit-btn" >Thanh toán</button>
